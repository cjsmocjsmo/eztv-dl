...
class PercyJacksonAndTheOlympians:
    def __init__(self, args, cwd):
        self.args = args
        self.PERCYJACKSONANDTHEOLYMPIANS = re.compile(r"PercyJacksonAndTheOlympians")
        self.PERCYJACKSONANDTHEOLYMPIANS_SEA = "s02e05"
    ...
    def search_percyjacksonandtheolympians_ez(self):
        try:
            r1 = requests.get(self.PERCYJACKSONANDTHEOLYMPIANS_EZ_1)
            r1_resp = r1.status_code
            count = 0
            if r1_resp == 200:
                p1_list = Search().ez_search_for_new_episode(r1.text, self.PERCYJACKSONANDTHEOLYMPIANS_SEA, self.PERCYJACKSONANDTHEOLYMPIANS_SEA_REG)
                resp1080p = len(p1_list[0])
                resp720p = len(p1_list[1])
                count += resp1080p + resp720p
                print("\nEZ PercyJacksonAndTheOlympians {} => \n\tstatus: {}, \n\t1080p: {}\n\t720p: {}".format(self.PERCYJACKSONANDTHEOLYMPIANS_SEA, r1_resp, resp1080p, resp720p))

            else:
                print("\nEZ PercyJacksonAndTheOlympians {} => status: {}".format(self.PERCYJACKSONANDTHEOLYMPIANS_SEA, r1_resp))
            return count
        except requests.exceptions.ConnectionError:
            print("PercyJacksonAndTheOlympians unable to connect to EZTV")
            return 0

    def search_percyjacksonandtheolympians(self):
        if self.args.eztv:
