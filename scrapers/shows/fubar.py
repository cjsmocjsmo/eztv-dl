...
class Fubar:
    def __init__(self, args, cwd):
        ...
        self.FUBAR_logger = logging.getLogger(__name__)
        self.FUBAR_logger.setLevel(logging.DEBUG)
        self.file_handler = None
        addr1 = cwd + '/logs/fubar.log'
        if os.path.exists(addr1):
            self.file_handler = logging.FileHandler(addr1, mode='w')
            self.file_handler.setFormatter(logging.Formatter('%(asctime)s %(name)s %(levelname)s %(message)s'))
            self.FUBAR_logger.addHandler(self.file_handler)
    ...
    def search_fubar_ez(self):
        try:
            ...
            self.FUBAR_logger.error("fubar unable to connect to EZTV")
            return 0
    ...
    def search_fubar_ka(self):
        try:
            ...
            if r2_resp == 200 and r3_resp == 200:
                ...
                self.FUBAR_logger.info("KA fubar {} => \n\tstatus: {}\n\t1080p: {}\n\t720p: {}".format(self.FUBAR_SEA, r3_resp, res1[0], res1[1]))
            else:
                print("KA fubar {} => status: {}".format(self.FUBAR_SEA, r3_resp))
                self.FUBAR_logger.info("KA fubar {} => status: {}".format(self.FUBAR_SEA, r3_resp))
            return count
        except requests.exceptions.ConnectionError as e:
            print(e)
            self.FUBAR_logger.error(e)
            return 0
