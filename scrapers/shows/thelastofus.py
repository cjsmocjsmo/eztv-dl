...
class TheLastOfUs:
    def __init__(self, args, cwd):
        ...
        self.thelastofus_logger = logging.getLogger(__name__)
        self.thelastofus_logger.setLevel(logging.DEBUG)
        self.file_handler = None
        addr1 = cwd + '/logs/thelastofus.log'
        if os.path.exists(addr1):
            self.file_handler = logging.FileHandler(addr1, mode='w')
            self.file_handler.setFormatter(logging.Formatter('%(asctime)s %(name)s %(levelname)s %(message)s'))
            self.thelastofus_logger.addHandler(self.file_handler)
        else:
            # create addr1
    ...
    def search_thelastofus_ez(self):
        try:
            ...
            self.thelastofus_logger.error("thelastofus unable to connect to EZTV")
            return 0
    ...
    def search_thelastofus_ka(self):
        try:
            ...
            if r2_resp == 200 and r3_resp == 200:
                ...
                self.thelastofus_logger.info("KA thelastofus {} => \n\tstatus: {}\n\t1080p: {}\n\t720p: {}".format(self.thelastofus_SEA, r3_resp, res1[0], res1[1]))
            else:
                print("KA thelastofus {} => status: {}".format(self.thelastofus_SEA, r3_resp))
                self.thelastofus_logger.info("KA thelastofus {} => status: {}".format(self.thelastofus_SEA, r3_resp))
            return count
        except requests.exceptions.ConnectionError as e:
            print(e)
            self.thelastofus_logger.error(e)
            return 0
